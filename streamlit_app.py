import streamlit as stimport pandas as pdimport numpy as npimport yfinance as yfimport statsmodels.api as smfrom scipy import statsfrom groq import Groqfrom datetime import dateimport reimport jsonimport base64import warningswarnings.filterwarnings("ignore")
...
AQR_QMJ_URL = "https://images.aqr.com/-/media/AQR/Documents/Insights/Data-Sets/Quality-Minus-Junk-Factors-Monthly.xlsx"AQR_BAB_URL = "https://images.aqr.com/-/media/AQR/Documents/Insights/Data-Sets/Betting-Against-Beta-Equity-Factors-Monthly.xlsx"

st.set_page_config(page_title="Factor Regression", page_icon="📈", layout="wide")

def get_bg_base64():try:with open("ranger-4df6c1b6.png", "rb") as f:return base64.b64encode(f.read()).decode()except Exception:return None

_BG = get_bg_base64()

_BG_CSS = (f"""background-image:linear-gradient(rgba(4,52,44,0.88), rgba(4,52,44,0.88)),url("data/png;base64,{_BG}");background-size: cover;background-position: center;background-attachment: fixed;"""if _BG else"""background: #1a0933;background-image:radial-gradient(ellipse at 15% 0%, rgba(29,158,117,0.18) 0%, transparent 55%),radial-gradient(ellipse at 85% 100%, rgba(15,110,86,0.14) 0%, transparent 55%),radial-gradient(ellipse at 50% 50%, rgba(4,52,44,0.95) 0%, transparent 100%);""")
...
AQR_FACTORS = ["QMJ", "BAB"]ALL_FACTORS = FF_FACTORS + AQR_FACTORS

Which factors come from AQR (used for badge rendering)

AQR_FACTOR_SET = set(AQR_FACTORS)

def sig_badge(p):if p < 0.001: return '★★★'elif p < 0.01: return '★★★'elif p < 0.05: return '★★'elif p < 0.10: return '★'else: return 'n.s.'

def row_class(name, p):if name == "const": return "factor-row alpha"if p < 0.05: return "factor-row sig"if p < 0.10: return "factor-row marg"return "factor-row insig"

def fmt_beta(v):color = "#34d399" if v > 0 else "#f87171"return f'{v:+.4f}'

def fmt_pval(p):if p < 0.001: color = "#34d399"elif p < 0.05: color = "#6ee7b7"elif p < 0.10: color = "#fbbf24"else: color = "#5DCAA5"return f'{p:.4f}'

def fmt_tstat(t):color = "#34d399" if abs(t) > 1.96 else "#5DCAA5"return f'{t:+.3f}'

FACTOR_NAMES = {"const":  "Alpha (α)","Mkt-RF": "Market (β)","SMB":    "Size","HML":    "Value","RMW":    "Profitability","CMA":    "Investment","Mom":    "Momentum","QMJ":    "Quality (QMJ)","BAB":    "Betting Agst Beta",}

FACTOR_DESCRIPTIONS = {"Mkt-RF": "Market risk premium (excess return of market over risk-free rate)","SMB":    "Size factor — Small Minus Big (small-cap vs large-cap premium)","HML":    "Value factor — High Minus Low (value vs growth premium)","RMW":    "Profitability factor — Robust Minus Weak (profitable vs unprofitable firms)","CMA":    "Investment factor — Conservative Minus Aggressive (low vs high investment firms)","Mom":    "Momentum factor (past winners vs past losers)","QMJ":    "Quality Minus Junk (AQR) — long high-quality stocks, short low-quality/junk stocks","BAB":    "Betting Against Beta (AQR) — long low-beta stocks, short high-beta stocks (leverage-constrained anomaly)",}

FACTOR_COLORS = {"Mkt-RF": "#60a5fa", "SMB": "#34d399", "HML": "#fbbf24","RMW": "#a78bfa", "CMA": "#f97316", "Mom": "#ec4899","QMJ":  "#facc15",   # gold — AQR quality"BAB":  "#38bdf8",   # sky blue — AQR BAB"const": "#94a3b8",}

@st.cache_data(show_spinner=False)def load_factors():ff = pd.read_csv(FF_URL, index_col=0)ff.columns = ff.columns.str.strip()ff.index = ff.index.astype(str).str.strip()ff = ff.drop(columns=[c for c in ff.columns if "Unnamed" in str(c)])ff = ff[ff.index.str.match(r"^\d{6}$")].copy()ff.index = pd.to_datetime(ff.index, format="%Y%m").to_period("M")return ff

@st.cache_data(show_spinner=False)def load_aqr_factors():"""Load QMJ and BAB monthly factor returns from AQR's public Excel files.Returns a DataFrame indexed by Period("M") with columns QMJ and BAB (as decimals).Falls back to empty DataFrame on any error."""import io
...
def merge_all_factors(ff_df, aqr_df, selected_factors):"""Merge FF and AQR factor DataFrames, return only the selected columns that exist."""combined = ff_df.copy()for col in AQR_FACTORS:if col in selected_factors and col in aqr_df.columns:combined[col] = aqr_df[col]return combined

def get_live_price(ticker_symbol):t = yf.Ticker(ticker_symbol)try:fi = t.fast_infolp = getattr(fi, "last_price", None) or getattr(fi, "lastPrice", None)pc = getattr(fi, "previous_close", None) or getattr(fi, "previousClose", None)cy = getattr(fi, "currency", "USD") or "USD"if lp and pc:return float(lp), float(pc), cyexcept Exception:passtry:hist = t.history(period="5d")if not hist.empty and len(hist) >= 2:return float(hist["Close"].iloc[-1]), float(hist["Close"].iloc[-2]), "USD"elif not hist.empty:p = float(hist["Close"].iloc[-1])return p, p, "USD"except Exception:passtry:info = t.infolp = info.get("currentPrice") or info.get("regularMarketPrice")pc = info.get("previousClose") or info.get("regularMarketPreviousClose")cy = info.get("currency", "USD")if lp and pc:return float(lp), float(pc), cyexcept Exception:passraise ValueError("All price-fetch layers failed")

def _call_groq(client, system_msg, user_msg, max_tokens=800):response = client.chat.completions.create(model="llama-3.3-70b-versatile",messages=[{"role": "system", "content": system_msg}, {"role": "user", "content": user_msg}],max_tokens=max_tokens, temperature=0.2,response_format={"type": "json_object"},)raw = response.choices[0].message.content.strip()raw = re.sub(r"^json\s*", "", raw)
    raw = re.sub(r"\s*$", "", raw)return json.loads(raw)

def _get_stock_context(ticker):ctx = {}try:t_obj = yf.Ticker(ticker)info = t_obj.infofor k, v in [("current_price", info.get("currentPrice") or info.get("regularMarketPrice")),("market_cap", info.get("marketCap")), ("sector", info.get("sector", "Unknown")),("industry", info.get("industry", "Unknown")), ("pe_ratio", info.get("trailingPE")),("pb_ratio", info.get("priceToBook")), ("ps_ratio", info.get("priceToSalesTrailing12Months")),("revenue_growth", info.get("revenueGrowth")), ("earnings_growth", info.get("earningsGrowth")),("profit_margin", info.get("profitMargins")), ("gross_margin", info.get("grossMargins")),("roe", info.get("returnOnEquity")), ("debt_to_equity", info.get("debtToEquity")),("free_cashflow", info.get("freeCashflow")), ("dividend_yield", info.get("dividendYield")),("beta_1y", info.get("beta")), ("52w_high", info.get("fiftyTwoWeekHigh")),("52w_low", info.get("fiftyTwoWeekLow")), ("short_ratio", info.get("shortRatio")),("short_pct_float", info.get("shortPercentOfFloat")), ("analyst_target", info.get("targetMeanPrice")),("rec_mean", info.get("recommendationMean")), ("num_analysts", info.get("numberOfAnalystOpinions")),("forward_pe", info.get("forwardPE")), ("peg_ratio", info.get("pegRatio")),("enterprise_ev_ebitda", info.get("enterpriseToEbitda")),("held_pct_inst", info.get("heldPercentInstitutions")),("company_name", info.get("longName", ticker)),]:ctx[k] = vfor period, key in [("ytd", "ytd_return"), ("1mo", "return_1m"), ("3mo", "return_3m")]:h = t_obj.history(period=period)if not h.empty and len(h) >= 2:ctx[key] = (h["Close"].iloc[-1] / h["Close"].iloc[0]) - 1except Exception:passreturn ctx

def _fmt_ctx(ctx):lines = []if ctx.get("company_name"):    lines.append(f"Company: {ctx['company_name']}")if ctx.get("sector"):          lines.append(f"Sector: {ctx['sector']} | Industry: {ctx.get('industry','?')}")if ctx.get("current_price"):   lines.append(f"Price: ${ctx['current_price']:.2f}")if ctx.get("market_cap"):      lines.append(f"Market Cap: ${ctx['market_cap']/1e9:.1f}B")if ctx.get("ytd_return") is not None: lines.append(f"YTD Return: {ctx['ytd_return']:+.1%}")if ctx.get("return_1m") is not None:  lines.append(f"1-Month Return: {ctx['return_1m']:+.1%}")if ctx.get("return_3m") is not None:  lines.append(f"3-Month Return: {ctx['return_3m']:+.1%}")if ctx.get("52w_high") and ctx.get("52w_low"):lines.append(f"52-Week Range: ${ctx['52w_low']:.2f} - ${ctx['52w_high']:.2f}")if ctx.get("pe_ratio"):        lines.append(f"P/E (TTM): {ctx['pe_ratio']:.1f}")if ctx.get("forward_pe"):      lines.append(f"Forward P/E: {ctx['forward_pe']:.1f}")if ctx.get("pb_ratio"):        lines.append(f"P/B: {ctx['pb_ratio']:.2f}")if ctx.get("peg_ratio"):       lines.append(f"PEG: {ctx['peg_ratio']:.2f}")if ctx.get("enterprise_ev_ebitda"): lines.append(f"EV/EBITDA: {ctx['enterprise_ev_ebitda']:.1f}")if ctx.get("revenue_growth") is not None:  lines.append(f"Revenue Growth (YoY): {ctx['revenue_growth']:+.1%}")if ctx.get("earnings_growth") is not None: lines.append(f"Earnings Growth (YoY): {ctx['earnings_growth']:+.1%}")if ctx.get("profit_margin") is not None:   lines.append(f"Net Margin: {ctx['profit_margin']:.1%}")if ctx.get("gross_margin") is not None:    lines.append(f"Gross Margin: {ctx['gross_margin']:.1%}")if ctx.get("roe") is not None:             lines.append(f"ROE: {ctx['roe']:.1%}")if ctx.get("debt_to_equity") is not None:  lines.append(f"Debt/Equity: {ctx['debt_to_equity']:.2f}")if ctx.get("beta_1y"):         lines.append(f"1Y Beta (Yahoo): {ctx['beta_1y']:.2f}")if ctx.get("dividend_yield"):  lines.append(f"Dividend Yield: {ctx['dividend_yield']:.2%}")if ctx.get("short_pct_float") is not None: lines.append(f"Short % of Float: {ctx['short_pct_float']:.1%}")if ctx.get("short_ratio"):     lines.append(f"Short Ratio (days): {ctx['short_ratio']:.1f}")if ctx.get("analyst_target"):  lines.append(f"Analyst Mean PT: ${ctx['analyst_target']:.2f} ({ctx.get('num_analysts','?')} analysts)")if ctx.get("rec_mean"):rec_map = {1:"Strong Buy", 2:"Buy", 3:"Hold", 4:"Underperform", 5:"Sell"}lines.append(f"Consensus: {rec_map.get(round(ctx['rec_mean']), str(ctx['rec_mean']))}")if ctx.get("held_pct_inst") is not None: lines.append(f"Institutional Ownership: {ctx['held_pct_inst']:.1%}")return "\n".join(lines)

def _factor_prompt(ticker, factor_code, factor_name, beta, p_value, significant, description, ctx_str):today = date.today().strftime("%B %d, %Y")is_aqr = factor_code in AQR_FACTOR_SETaqr_note = ("\nThis is an AQR factor. For QMJ, discuss quality metrics (profitability, earnings quality, safety, payout). ""For BAB, discuss the low-beta/low-volatility anomaly and leverage constraints.") if is_aqr else ""system = ("You are a senior quantitative analyst at a top-tier hedge fund. ""You have been given LIVE fundamental and price data for the stock. Use it. ""Your analysis must be SPECIFIC to this exact ticker. ""You MUST respond with a single valid JSON object and nothing else.""You have to check various news sources and check all the finance related developments and any investments decided.""Make use of prnewswire to get the latest quarterly results and 8-k announcements to use more numbers.""But dont explicitly keep saying 8-k or pnr. Make heavy use of cnbc to get the company developments.""Analyze the War in Iran and take that into consideration also and its affect on the stock"+ aqr_note)sig_word = "statistically significant (p<0.05)" if significant else f"not statistically significant (p={p_value:.3f})"user = f"""Today is {today}. Analyze the {factor_name} ({factor_code}) loading for {ticker}.

=== LIVE STOCK DATA ==={ctx_str}

=== REGRESSION RESULT ===Factor: {factor_name} ({factor_code})Definition: {description}Beta: {beta:+.4f}P-value: {p_value:.4f} ({sig_word})

Return EXACTLY this JSON (no line breaks inside strings):{{"code": "{factor_code}","name": "{factor_name}","beta": {beta},"significant": {"true" if significant else "false"},"outlook": "<bullish|bearish|neutral|mixed>","what_it_means": "<2-3 sentences using ACTUAL numbers from the data above.>","current_macro_context": "<2-3 sentences on current macro environment relevant to THIS factor.>","recent_stock_news": "<2-3 sentences on the most recent material developments for {ticker}.>","forward_forecast": "<2 sentences: near-term directional call for {ticker} on this factor.>","key_risks": "<1-2 sentences: the single most specific risk that could flip this outlook.>"}}"""return system, user

def _summary_prompt(ticker, alpha_ann, alpha_p, r2, factor_summaries, ctx_str):today = date.today().strftime("%B %d, %Y")system = ("You are a senior portfolio strategist. ""Use specific numbers from the live data. ""Respond with a single valid JSON object only. No markdown.")sig_word = "statistically significant" if alpha_p < 0.05 else "not statistically significant"lines = "\n".join(f"- {f['name']}: beta={f['beta']:+.4f}, outlook={f.get('outlook','?')}" for f in factor_summaries)user = f"""Today: {today}. Stock: {ticker}Annualized alpha: {alpha_ann:+.2%} (p={alpha_p:.4f}, {sig_word})R2: {r2:.4f}Significant factor loadings:{lines if lines else "None"}

=== LIVE STOCK DATA ==={ctx_str}

Return EXACTLY this JSON:{{"alpha_analysis": "<Is alpha={alpha_ann:+.2%} meaningful for {ticker}? What explains it? What threatens persistence?>","positioning_context": "<Analyst consensus, institutional ownership, short interest, factor profile alignment.>","portfolio_verdict": "<Risk-adjusted verdict as of {today}. Best-case and worst-case. One-line bottom line: buy/hold/avoid.>"}}"""return system, user

def get_ai_insight(ticker, model_result, available, alpha_ann, alpha_p, r2, n, start_date, end_date):api_key = st.secrets.get("GROQ_API_KEY", None)if not api_key:return None, "No API key found. Add GROQ_API_KEY to Streamlit secrets."ctx = _get_stock_context(ticker)ctx_str = _fmt_ctx(ctx)try:client = Groq(api_key=api_key)sig_factors = [f for f in available if float(model_result.pvalues[f]) < 0.05]if not sig_factors:sys_msg, usr_msg = _summary_prompt(ticker, alpha_ann, alpha_p, r2, [], ctx_str)try:summary = _call_groq(client, sys_msg, usr_msg, max_tokens=700)except Exception as e:summary = {"alpha_analysis": f"Summary unavailable: {e}", "positioning_context": "", "portfolio_verdict": ""}return {"factors": [], **summary}, Nonefactors_out = []errors = []for f in sig_factors:beta = float(model_result.params[f])pval = float(model_result.pvalues[f])sys_msg, usr_msg = _factor_prompt(ticker, f, FACTOR_NAMES.get(f, f), beta, pval, True, FACTOR_DESCRIPTIONS.get(f, f), ctx_str=ctx_str)try:result = _call_groq(client, sys_msg, usr_msg, max_tokens=900)result["beta"] = betaresult["significant"] = Truefactors_out.append(result)except Exception as e:factors_out.append({"code": f, "name": FACTOR_NAMES.get(f, f), "beta": beta, "significant": True,"outlook": "neutral", "what_it_means": "Analysis unavailable.","current_macro_context": f"Error: {e}", "recent_stock_news": "","forward_forecast": "", "key_risks": ""})errors.append(f"{f}: {e}")sys_msg, usr_msg = _summary_prompt(ticker, alpha_ann, alpha_p, r2, factors_out, ctx_str)try:summary = _call_groq(client, sys_msg, usr_msg, max_tokens=700)except Exception as e:summary = {"alpha_analysis": f"Summary unavailable: {e}", "positioning_context": "", "portfolio_verdict": ""}errors.append(f"summary: {e}")return {"factors": factors_out, **summary}, ("; ".join(errors) if errors else None)except Exception as e:return None, str(e)

def render_ai_insight(ticker, insight_data):factors = insight_data.get("factors", [])alpha_analysis = insight_data.get("alpha_analysis", "")positioning_context = insight_data.get("positioning_context", "")portfolio_verdict = insight_data.get("portfolio_verdict", "")
...
@st.cache_data(show_spinner=False)def fetch_monthly_returns(ticker_sym, start_str, end_str):try:raw = yf.download(ticker_sym, start=start_str + "-01", end=end_str + "-28",auto_adjust=True, progress=False)if raw.empty:return None, f"No data for {ticker_sym}"close = raw["Close"]if isinstance(close, pd.DataFrame):close = close.iloc[:, 0]monthly = close.resample("ME").last()returns = monthly.pct_change().dropna()returns.index = returns.index.to_period("M")return returns, Noneexcept Exception as e:return None, str(e)

@st.cache_data(show_spinner=False)def run_single_regression(ticker_sym, start_str, end_str, hac_lags, ff_key, selected_factors_key):try:ff_raw  = load_factors()aqr_raw = load_aqr_factors()ff_raw  = merge_all_factors(ff_raw, aqr_raw, list(selected_factors_key))
...
def build_true_portfolio_model(port_results, weights, available_factors, start_str, end_str, hac_lags):try:ff_raw  = load_factors()aqr_raw = load_aqr_factors()ff_raw  = merge_all_factors(ff_raw, aqr_raw, available_factors)
...
def render_portfolio_attribution(port_results, weights, available_factors, true_port, start_str, end_str, hac_lags):tickers  = list(port_results.keys())n_stocks = len(tickers)
...
for key, default in [("run", False), ("ai_insight", None), ("ai_error", None),("port_run", False), ("port_results", None),("single_stock_cache", None), ("portfolio_cache", None),("active_mode", "Single Stock"),]:if key not in st.session_state:st.session_state[key] = default

if "selected_factors" not in st.session_state:st.session_state["selected_factors"] = list(ALL_FACTORS)
...
st.markdown("# Factor Regression")st.markdown('''FF5 + MOMENTUM + AQR (QMJ · BAB) · NEWEY-WEST ROBUST STANDARD ERRORS',unsafe_allow_html=True)st.markdown("---")
...
if not st.session_state["run"] and not st.session_state["port_run"] and st.session_state["single_stock_cache"] is None and st.session_state["portfolio_cache"] is None:if current_mode == "Single Stock":st.markdown('''Single Stock Mode''1. Enter a ticker2. Set date range''3. Select factors — including AQR QMJ & BAB''4. Click ▶ RUN REGRESSION''Factor analysis runs only on statistically significant loadings (p<0.05).''', unsafe_allow_html=True)else:st.markdown('''Portfolio Attribution Mode''1. Add your holdings (ticker + amount)2. Set date range''3. Select factors — including AQR QMJ & BAB''4. Click ▶ RUN PORTFOLIO ATTRIBUTION''', unsafe_allow_html=True)st.stop()
...
if current_mode == "Portfolio Attribution":if st.session_state["portfolio_cache"] is not None and not st.session_state.get("port_run"):cache = st.session_state["portfolio_cache"]st.markdown("## Portfolio Factor Attribution")st.markdown(f''f'{len(cache["weights"])} holdings · {len(cache["ordered_factors"])} factors · HAC SE · {cache["port_start"]} → {cache["port_end"]}'f'', unsafe_allow_html=True)render_portfolio_attribution(cache["port_results"], cache["weights"], cache["ordered_factors"],cache.get("true_port"), str(cache["port_start"])[:7], str(cache["port_end"])[:7],cache.get("hac_lags", 3))st.stop()
...
def _render_ols_summary(ols_text):escaped = ols_text.replace("&", "&").replace("<", "<").replace(">", ">")st.markdown(f'{escaped}',unsafe_allow_html=True)
