...
.metric-card {{
    ...
    border-left: 2px solid #f87171; border-radius: 12px; padding: 14px 16px;
    margin-bottom: 10px;
}}
...
.factor-row.sig   {{ border-left: 2px solid #f87171; }}
...
.badge-001 {{ background: rgba(52,211,153,0.15); color: #f87171; }}
...
.badge-ns  {{ background: rgba(255,255,255,0.03); color: #f87171; }}
...
.diag-pass {{ color: #f87171; }} .diag-fail {{ color: #f87171; }} .diag-warn {{ color: #fbbf24; }}
...
.factor-insight-card.positive {{ border-left: 3px solid #f87171; }}
...
.fi-beta.pos {{ background: rgba(52,211,153,0.12); color: #f87171; }}
...
.fi-outlook.bullish  {{ background: rgba(52,211,153,0.1); color: #f87171; }}
...
.port-attr-beta.pos {{ color: #f87171; }}
...
def fmt_tstat(t):
    color = "#f87171" if abs(t) > 1.96 else "#5DCAA5"
...
FACTOR_COLORS = {
    "Mkt-RF": "#60a5fa", "SMB": "#f87171", "HML": "#fbbf24",
    ...
    for f in factors_to_show:
        ...
        for label, months in windows:
            if len(series) >= months:
                ...
                color = "#f87171" if cumret > 0 else "#f87171"
...
def render_active_exposure(entity_params, bench_params, available_factors, bench_ticker,
    ...
    for f in ["const"] + factors_to_show:
        ...
        active = eb - bb
        ec = "#f87171" if eb >= 0 else "#f87171"
        bc = "#f87171" if bb >= 0 else "#f87171"
        ac = "#f87171" if active >= 0 else "#f87171"
        aqr_tag = ' <span class="aqr-badge">AQR</span>' if f in AQR_FACTOR_SET else ""
...
def render_portfolio_attribution(port_results, weights, available_factors, true_port, start_str, end_str, hac_lags):
    ...
    alpha_color = "#f87171" if port_alpha_ann > 0 else "#f87171"
    ...
    for tkr in tickers:
        ...
        w       = weights[tkr]
        alpha_c = "#f87171" if res["alpha_ann"] > 0 else "#f87171"
    ...
    alpha_c = "#f87171" if port_alpha_ann > 0 else "#f87171"
    ...
    for f in available_factors:
        ...
        disp = np.std(vals)
        rc   = "#f87171" if disp > 0.5 else "#fbbf24" if disp > 0.2 else "#f87171"
...
if current_mode == "Single Stock":
    ...
    try:
        ...
        try:
            ...
            _clr = "#f87171" if _chg >= 0 else "#f87171"; _arrow = "&#9650;" if _chg >= 0 else "&#9660;"
            _price_html = (
        ...
        <div class="metric-card {'green' if alpha_ann > 0 else 'red'}">
          ...
          <div class="metric-value" style="color:{'#f87171' if alpha_ann>0 else '#f87171'}">{alpha_ann:+.2%}</div>
        ...
        ir_color = "#f87171" if not np.isnan(ir) and ir > 0.5 else "#fbbf24" if not np.isnan(ir) and ir > 0 else "#f87171"
        ...
        legend_html = (
            ...
            f' | <span style="color:#f87171;">{n_sig} of {len(available)} factors significant at p&lt;0.05</span>'
        ...
        for name in ["const"] + available:
            ...
            spans_zero = lo < 0 < hi
            bar_color  = "#f87171" if b > 0 and not spans_zero else "#f87171" if b < 0 and not spans_zero else "#6b7280"
            aqr_tag = ' <span class="aqr-badge">AQR</span>' if name in AQR_FACTOR_SET else ""
        ...
        for col, v in vif_data.items():
            vc = "#f87171" if v < 5 else "#fbbf24" if v < 10 else "#f87171"
