...
.metric-card {{
    ...
    border-left: 2px solid #0a1f1a; border-radius: 12px; padding: 14px 16px;
    margin-bottom: 10px;
}}
...
.factor-row.sig   {{ border-left: 2px solid #0a1f1a; }}
...
.badge-001 {{ background: rgba(52,211,153,0.15); color: #0a1f1a; }}
...
.badge-ns  {{ background: rgba(255,255,255,0.03); color: #0a1f1a; }}
...
.diag-pass {{ color: #0a1f1a; }} .diag-fail {{ color: #f87171; }} .diag-warn {{ color: #fbbf24; }}
...
.factor-insight-card.positive {{ border-left: 3px solid #0a1f1a; }}
...
.fi-beta.pos {{ background: rgba(52,211,153,0.12); color: #0a1f1a; }}
...
.fi-outlook.bullish  {{ background: rgba(52,211,153,0.1); color: #0a1f1a; }}
...
.port-attr-beta.pos {{ color: #0a1f1a; }}
...
def fmt_tstat(t):
    color = "#0a1f1a" if abs(t) > 1.96 else "#5DCAA5"
...
FACTOR_COLORS = {
    "Mkt-RF": "#60a5fa", "SMB": "#0a1f1a", "HML": "#fbbf24",
    ...
    for f in factors_to_show:
        ...
        for label, months in windows:
            if len(series) >= months:
                ...
                color = "#0a1f1a" if cumret > 0 else "#f87171"
...
def render_active_exposure(entity_params, bench_params, available_factors, bench_ticker,
    ...
    for f in ["const"] + factors_to_show:
        ...
        active = eb - bb
        ec = "#0a1f1a" if eb >= 0 else "#f87171"
        bc = "#0a1f1a" if bb >= 0 else "#f87171"
        ac = "#0a1f1a" if active >= 0 else "#f87171"
        aqr_tag = ' <span class="aqr-badge">AQR</span>' if f in AQR_FACTOR_SET else ""
...
def render_portfolio_attribution(port_results, weights, available_factors, true_port, start_str, end_str, hac_lags):
    ...
    alpha_color = "#0a1f1a" if port_alpha_ann > 0 else "#f87171"
    ...
    for tkr in tickers:
        ...
        w       = weights[tkr]
        alpha_c = "#0a1f1a" if res["alpha_ann"] > 0 else "#f87171"
    ...
    alpha_c = "#0a1f1a" if port_alpha_ann > 0 else "#f87171"
    ...
    for f in available_factors:
        ...
        disp = np.std(vals)
        rc   = "#f87171" if disp > 0.5 else "#fbbf24" if disp > 0.2 else "#0a1f1a"
...
if current_mode == "Single Stock":
    ...
    try:
        ...
        try:
            ...
            _clr = "#0a1f1a" if _chg >= 0 else "#f87171"; _arrow = "&#9650;" if _chg >= 0 else "&#9660;"
            _price_html = (
        ...
        <div class="metric-card {'green' if alpha_ann > 0 else 'red'}">
          ...
          <div class="metric-value" style="color:{'#0a1f1a' if alpha_ann>0 else '#f87171'}">{alpha_ann:+.2%}</div>
        ...
        ir_color = "#0a1f1a" if not np.isnan(ir) and ir > 0.5 else "#fbbf24" if not np.isnan(ir) and ir > 0 else "#f87171"
        ...
        legend_html = (
            ...
            f' | <span style="color:#0a1f1a;">{n_sig} of {len(available)} factors significant at p&lt;0.05</span>'
        ...
        for name in ["const"] + available:
            ...
            spans_zero = lo < 0 < hi
            bar_color  = "#0a1f1a" if b > 0 and not spans_zero else "#f87171" if b < 0 and not spans_zero else "#6b7280"
            aqr_tag = ' <span class="aqr-badge">AQR</span>' if name in AQR_FACTOR_SET else ""
        ...
        for col, v in vif_data.items():
            vc = "#0a1f1a" if v < 5 else "#fbbf24" if v < 10 else "#f87171"
