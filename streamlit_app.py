...
.metric-card {{
    ...
    border-left: 2px solid #04342c; border-radius: 12px; padding: 14px 16px;
    margin-bottom: 10px;
}}
.metric-card.red  {{ border-left-color: #04342c; }}
...
.factor-row.sig   {{ border-left: 2px solid #04342c; }}
...
.badge-001 {{ background: rgba(52,211,153,0.15); color: #04342c; }}
...
.badge-ns  {{ background: rgba(255,255,255,0.03); color: #04342c; }}
...
.diag-pass {{ color: #04342c; }} .diag-fail {{ color: #04342c; }} .diag-warn {{ color: #fbbf24; }}
...
.factor-insight-card.positive {{ border-left: 3px solid #04342c; }}
.factor-insight-card.negative {{ border-left: 3px solid #04342c; }}
...
.fi-beta.pos {{ background: rgba(52,211,153,0.12); color: #04342c; }}
.fi-beta.neg {{ background: rgba(248,113,113,0.12); color: #04342c; }}
...
.fi-outlook.bullish  {{ background: rgba(52,211,153,0.1); color: #04342c; }}
.fi-outlook.bearish  {{ background: rgba(248,113,113,0.1); color: #04342c; }}
...
.error-box {{
    ...
    font-family: 'JetBrains Mono', monospace; font-size: 13px; color: #04342c;
}}
.info-box {{
...
.port-attr-beta.pos {{ color: #04342c; }}
.port-attr-beta.neg {{ color: #04342c; }}
...
def fmt_tstat(t):
    color = "#04342c" if abs(t) > 1.96 else "#5DCAA5"
...
FACTOR_COLORS = {
    "Mkt-RF": "#60a5fa", "SMB": "#04342c", "HML": "#fbbf24",
    ...
    for f in factors_to_show:
        ...
        for label, months in windows:
            if len(series) >= months:
                ...
                color = "#04342c" if cumret > 0 else "#04342c"
...
def render_active_exposure(entity_params, bench_params, available_factors, bench_ticker,
    ...
    for f in ["const"] + factors_to_show:
        ...
        active = eb - bb
        ec = "#04342c" if eb >= 0 else "#04342c"
        bc = "#04342c" if bb >= 0 else "#04342c"
        ac = "#04342c" if active >= 0 else "#04342c"
        aqr_tag = ' <span class="aqr-badge">AQR</span>' if f in AQR_FACTOR_SET else ""
...
def render_portfolio_attribution(port_results, weights, available_factors, true_port, start_str, end_str, hac_lags):
    ...
    alpha_color = "#04342c" if port_alpha_ann > 0 else "#04342c"
    ...
    for tkr in tickers:
        ...
        w       = weights[tkr]
        alpha_c = "#04342c" if res["alpha_ann"] > 0 else "#04342c"
    ...
    alpha_c = "#04342c" if port_alpha_ann > 0 else "#04342c"
    ...
    for f in available_factors:
        ...
        disp = np.std(vals)
        rc   = "#04342c" if disp > 0.5 else "#fbbf24" if disp > 0.2 else "#04342c"
...
with st.sidebar:
    ...
    if st.session_state["selected_factors"]:
        ...
        st.markdown(
            '<div style="font-family:\'JetBrains Mono\',monospace;font-size:12px;color:#04342c;margin-top:6px;">'
...
if current_mode == "Single Stock":
    ...
    try:
        ...
        try:
            ...
            _clr = "#04342c" if _chg >= 0 else "#04342c"; _arrow = "&#9650;" if _chg >= 0 else "&#9660;"
            _price_html = (
        ...
        <div class="metric-card {'green' if alpha_ann > 0 else 'red'}">
          ...
          <div class="metric-value" style="color:{'#04342c' if alpha_ann>0 else '#04342c'}">{alpha_ann:+.2%}</div>
        ...
        ir_color = "#04342c" if not np.isnan(ir) and ir > 0.5 else "#fbbf24" if not np.isnan(ir) and ir > 0 else "#04342c"
        ...
        legend_html = (
            ...
            f' | <span style="color:#04342c;">{n_sig} of {len(available)} factors significant at p&lt;0.05</span>'
        ...
        for name in ["const"] + available:
            ...
            spans_zero = lo < 0 < hi
            bar_color  = "#04342c" if b > 0 and not spans_zero else "#04342c" if b < 0 and not spans_zero else "#6b7280"
            aqr_tag = ' <span class="aqr-badge">AQR</span>' if name in AQR_FACTOR_SET else ""
        ...
        for col, v in vif_data.items():
            vc = "#04342c" if v < 5 else "#fbbf24" if v < 10 else "#04342c"
