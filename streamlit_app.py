...
try:
    ...
    for name in ["const"] + available:
        ...
        <div class="{row_class(name, p)}">
          <div style="color:#e8e8f0;font-weight:600">{FACTOR_NAMES.get(name, name)}</div>
          <div>{fmt_beta(b)}</div>
    ...
    st.markdown(
        f'<div style="font-family:\'IBM Plex Mono\',monospace;font-size:10px;color:#374151;margin-top:6px;">'
    ...
    for name in ["const"] + available:
        ...
        ci_html += f"""
        <div style="background:#12121a;border:1px solid #1e1e2e;border-radius:4px;padding:12px 16px;min-width:160px;flex:1;">
          <div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#4b5563;margin-bottom:6px;">{FACTOR_NAMES.get(name, name)}</div>
          <div style="font-family:'IBM Plex Mono',monospace;font-size:13px;font-weight:600;color:{bar_color};">{b:+.4f}</div>
          <div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#374151;margin-top:3px;">[{lo:+.4f}, {hi:+.4f}]</div>
          <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#374151;margin-top:5px;">{'spans zero' if spans_zero else 'excl. zero'}</div>
        </div>"""
    ci_html += '</div>'
    ...
    for col, v in vif_data.items():
        ...
        <div style="background:#12121a;border:1px solid #1e1e2e;border-radius:4px;padding:12px 16px;min-width:100px;">
          ...
          <div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#374151;margin-top:2px;">{'OK' if v < 5 else 'MODERATE' if v < 10 else 'HIGH'}</div>
        </div>"""
    vif_html += '</div>'
    ...
    if "Mkt-RF" in available and len(data) >= 36:
        ...
        if roll_betas:
            ...
            <svg viewBox="0 0 {W} {H+30}" xmlns="http://www.w3.org/2000/svg"
                 style="background:#12121a;border:1px solid #1e1e2e;border-radius:4px;width:100%;margin-bottom:8px;">
              <line x1="0" y1="{one_y}" x2="{W}" y2="{one_y}" stroke="#1e1e2e" stroke-width="1" stroke-dasharray="4,4"/>
              <polyline points="{pts}" fill="none" stroke="#60a5fa" stroke-width="2"/>
              <text x="6" y="{H+20}" fill="#4b5563" font-family="IBM Plex Mono,monospace" font-size="10">{roll_dates[0]}</text>
              <text x="{W-6}" y="{H+20}" fill="#4b5563" text-anchor="end" font-family="IBM Plex Mono,monospace" font-size="10">{roll_dates[-1]}</text>
              <text x="{W//2}" y="14" fill="#6b7280" text-anchor="middle" font-family="IBM Plex Mono,monospace" font-size="10">Current β = {roll_betas[-1]:.3f}</text>
